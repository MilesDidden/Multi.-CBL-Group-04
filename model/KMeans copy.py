...
def calc_street_distance_between_crime_and_officer(clustered_data, centroids):
    # Join the data
    df = clustered_data.merge(pd.DataFrame(centroids), how="left", left_on="cluster", right_index=True)

    # Download street network for the area
    G = ox.load_graphml("data/london_map_drive.graphml")

    # Precompute graph to speed up distance calculation
    G = ox.distance.add_edge_lengths(G)

    distances = []
    for idx, row in tqdm(df.iterrows(), total=len(df)):
        # Starting point (officer or initial lat/lon)
        orig_point = (row[0], row[1])
        # Target point (centroid lat/lon)
        dest_point = (row["latitude"], row["longitude"])
        ...
        try:
            ...
            # If no path is found, set a large value or NaN
    ...
    # Add new column
    df["street_distance_meters"] = distances

    # Calculate mean and max (ignore NaNs)
    mean_distance = df["street_distance_meters"].mean().round(decimals=3)
    max_distance = df["street_distance_meters"].max().round(decimals=3)
