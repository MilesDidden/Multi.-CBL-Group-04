...
class DBhandler:
    ...
    # Create database
    def init_db(self, db_loc: str="data/", db_name: str="crime_data_UK.db") -> None:

        if (not os.path.exists(db_loc)) or (not db_loc.endswith("/")):
            raise ValueError("Invalid folder location!\n" +
                             "Make sure the database location ends with '/'")

        if (not db_name.endswith(".db")):
            raise ValueError("Invalid database name! Make sure the name ends with '.db'")
        ...
        if os.path.exists(self.db_path):
            print("\nDatabase already exists! Skipping this step!\n")
        else:
    ...
    def open_connection_db(self) -> None:
        if self.con is not None:
            raise ValueError("Connection is open!")
    ...
    def close_connection_db(self) -> None:
        if self.con is None:
            raise ValueError("No current open connections!")
...
if __name__ == "__main__":
    ...
    db_handler.init_db(db_loc="data/", db_name="crime_data_UK.db")
    ...
    # path_to_district_kml_files = "data/force kmls/"
    ...
    db_handler.create_table(
        table_name='crime',
        columns={'db_id': 'INTEGER PRIMARY KEY AUTOINCREMENT',
                 'crime_id':'TEXT',
                 'month':'TEXT',
                 'reported_by':'TEXT',
                 'falls_within':'TEXT',
